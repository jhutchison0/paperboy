...
Paperboy is a **downstream consumer** of the `utils` template repo for doctrine (agent definitions, skills, commands, conventions). It does not propagate doctrine to other repos.
...
- **Adopt upstream doctrine selectively.** The agent framework, skills, command conventions, and doctrine artifacts come from `utils`. Adopt with intent; adapt or skip when paperboy's domain warrants.
...
- **Active work**: Honest pipeline status reporting (`PipelineResult.partial`), AgentRunner timeout diagnostics, failure-reason propagation, doctrine artifact adoption from upstream `utils`.
- **Tests**: 161 passing.
...
- **Known reliability concerns**: blog feed audit pending — 5 of 6 feeds returned 0 articles on 2026-05-19 (one parse error, four likely dormant).
...
- Pipeline test coverage stays comprehensive — every new sourcer/scorer/distillation feature ships with tests in `tests/` (the PostToolUse shift-left audit hook logs evidence to `.claude/audits/shift-left-violations.log`).
...
**Upstream doctrine source**: [utils](../utils) (`~/projects/github/utils`) propagates doctrine updates via `.claude/upstream-update.md`. Paperboy reviews each entry and adopts/adapts/skips per artifact. The 2026-05-19 doctrine cycle (this adoption) brought LANGUAGE.md, CONTEXT.md, the ADR system, SKILLS_FRAMEWORK v2, the shift-left audit hook, and three legacy single-file skills refactored into directory form.
